    ...
    function btnReplace() {
        ...
        if (cbPesquisaRegex) {
            ...
            if (cbDiferenciaLetras) {
                ...
                let lowerCaseValue = cell.value.toLowerCase();
                let lowerCaseSearch = searchValue.toLowerCase();
                let index = lowerCaseValue.indexOf(lowerCaseSearch);

                while (index !== -1) {
                    cell.value = cell.value.slice(0, index) + replaceValue + cell.value.slice(index + searchValue.length);
                    lowerCaseValue = cell.value.toLowerCase();
                    index = lowerCaseValue.indexOf(lowerCaseSearch, index + replaceValue.length);
                }

                cell = { value: cell.value, style: cell.style };
            }
        }
